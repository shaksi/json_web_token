...
class JsonAuth implements AuthenticationProviderInterface {
  ...
  public function authenticate(Request $request) {
    ...
    if ($this->flood->isAllowed('json_auth.failed_login_ip', $flood_config->get('ip_limit'), $flood_config->get('ip_window'))) {
      ...
      if ($account) {
        ...
        if ($this->flood->isAllowed('json_auth.failed_login_user', $flood_config->get('user_limit'), $flood_config->get('user_window'), $identifier)) {
          $uid = $this->userAuth->authenticate($username, $password);
          if ($uid) {
            $this->flood->clear('json_auth.failed_login_user', $identifier);
            return $this->entityManager->getStorage('user')->load($uid);
          }
          else {
            // Register a per-user failed login event.
            $this->flood->register('json_auth.failed_login_user', $flood_config->get('user_window'), $identifier);
          }
        }
      }
    }
    // Always register an IP-based failed login event.
    $this->flood->register('json_auth.failed_login_ip', $flood_config->get('ip_window'));
    return [];
  }
}
